...
**Files:**
- Modify: `cluster/config.go:78` (delete `loopback`), `cluster/config.go:32-59` (add field), `cluster/config.go:165-168`
- Modify: `cluster/up.go:461-469` (fill the field)
- Test: `cluster/config_test.go:391-396`, `cluster/up_test.go`
...
In `cluster/config_test.go`, replace the body of the SAN assertion at line 391-396 with a table test:
...
func TestCertSANComesFromAPIAddress(t *testing.T) {
	for _, addr := range []string{"127.0.0.1", "192.168.1.50"} {
		...
		if err != nil {
			t.Fatalf("GenerateConfig(%s): %s", addr, redactErr(err))
		}

		cfg, err := configFromBytes(t, g.ControlPlane)
		if err != nil {
...
Use the existing helper that parses generated bytes in `config_test.go` (it is used at line 391); if it is inline rather than a helper, inline the same parse here.
...
func TestEmptyConsoleArgEmitsNoKernelArgsAndLeavesUKIAlone(t *testing.T) {
	...
	g, err := GenerateConfig(in)
	if err != nil {
		t.Fatalf("GenerateConfig: %s", redactErr(err))
	}
	...
	if args := cfg.Machine().Install().ExtraKernelArgs(); len(args) != 0 {
		t.Errorf("extraKernelArgs = %v, want none\n"+
			"  reason: real hardware has a firmware-configured console; forcing one "+
			"derived from the HOST's architecture is how a node boots with a dead console", args)
	}
	...
	// node's boot path for no reason.
	if !strings.Contains(string(g.ControlPlane), "grubUseUKICmdline") {
		return // machinery did not set it for this contract; nothing to assert
	}

	if strings.Contains(string(g.ControlPlane), "grubUseUKICmdline: false") {
		t.Error("grubUseUKICmdline was forced false with no console arg to protect\n" +
...
func TestConsoleArgStillEmittedWhenSet(t *testing.T) {
	...
	if args := cfg.Machine().Install().ExtraKernelArgs(); !slices.Contains(args, "console=ttyS0") {
		t.Errorf("extraKernelArgs = %v, want console=ttyS0\n"+
...
func TestUpRendersSubstrateAndVersionFromOptions(t *testing.T) {
	f := newUpFixture(t) // existing fixture helper in up_test.go
	f.opts.Substrate = "baremetal, 192.168.1.50"
	f.opts.TalosVersion = "v1.13.7"
	f.opts.VersionSource = "the node's maintenance API"

	if err := Up(t.Context(), f.opts); err != nil {
		t.Fatalf("Up: %s", redactErr(err))
	}
	...
	if !strings.Contains(out, "baremetal, 192.168.1.50") {
		t.Errorf("step 1 did not print the caller's substrate line\n"+
			"  reason: cluster/ no longer knows what a hypervisor is; the caller "+
			"renders that line\n  got:\n%s", out)
	}

	if !strings.Contains(out, "the node's maintenance API -> v1.13.7") {
...
Add a compile-time assertion in `cluster/up_test.go` that the package does not import `platform` — the honest way is a `go list` check in Step 6 rather than a test, so do not fake it with a test here.
